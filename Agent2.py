...
class DistributedAgent():
    def __init__(self):
        ...
        self.__data_dir = 'data/'  #data를 저장할 경로
        self.__handle_dir = 'data/handle_image/'  #handle 이미지를 가져 올 경로
        self.__per_iter_epsilon_reduction = 0.003  #입실론 값을 조정 할 변수
        self.__min_epsilon = 0.1  #최소 입실론 값
        self.__max_epoch_runtime_sec = float(30) #최대 runtime시간 = 운행 시작 후 30초가 지나면 저장함.
        self.__replay_memory_size = 50
        self.__batch_size = 32
        self.__experiment_name = 'local_run'
        self.__train_conv_layers = False
        self.__epsilon = 1  #입실론 값
        self.__percent_full = 0
        self.__num_batches_run = 0
        self.__last_checkpoint_batch_count = 0
        self.__handles = {}
        self.__batch_update_frequency = 10  #배치 업데이트 주기(10번 이상 best가 생기지 않으면, 이전 best값으로 돌아간다.)
        ...
        #self.__best_epsilon = 1 #added 2021-03-10 by kim
    ...
    def __run_function(self):
        self.__model = RlModel(self.__weights_path, self.__train_conv_layers) #false, false로 ->network를 가진 모델이 생긴다.
        self.__connect_to_airsim() #에어심에 연결하기
    ...
    def __get_next_starting_point(self):
        ...
        #car_state = self.__car_client.getCarState() #자동차의 현재 state를 받아온다.
        ...
        random_interp = 0.15    # changed by GY 21-03-10 #0에 가까울 수록 커브길에 가까워진다.
        
        # Pick a random direction to face
        random_direction_interp = 0.4 # changed by GY 21-03-10 #0.5이하일땐 오른쪽을 바라보며 시작한다.
