...
class DistributedAgent():
    ...
    def __get_next_starting_point(self):
        ...
        #random_interp = (np.random.random_sample() * 0.4) + 0.3 #changed by KGY
        random_interp = 0.15 #0에 가까울수록, 곡선거리에 근접하게 시작한다.
        
        # Pick a random direction to face
        #random_direction_interp = np.random.random_sample() #changed by KGY
        random_direction_interp = 0.4 #0.5이전일때 오른쪽방향을 본다.
