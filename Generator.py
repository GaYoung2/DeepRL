...
class DriveDataGenerator(image.ImageDataGenerator):
    def __init__(self,
                 ...
                 brighten_range=0.0):
        super(DriveDataGenerator, self).__init__(rescale=rescale, horizontal_flip=horizontal_flip)
    ...
    def random_transform_with_states(self, x, seed=None):
        ...
        if self.horizontal_flip:
            if np.random.random() < 0.5:
                x = affine_transformations.flip_axis(x, img_col_axis)
                is_image_horizontally_flipped = True

        if self.vertical_flip:
            if np.random.random() < 0.5:
                x = affine_transformations.flip_axis(x, img_row_axis)
